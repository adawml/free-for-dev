...
$.crewFormLib = {
	...
	validateForm : function(options){
		...
		options = $.extend(true, defaults, $.crewFormLib.validateFormDefaults, globalValidateFormDefaults, options);
	...
	showMsg : function(target){
		...
		switch(options.msgType){
			...
			case "nepa" : $.crewFormLib.showMsgNepa(target); break;
		}
	}, 
		...
		for(var name in chkResult){
			for(var key in chkResult[name]){
				if(chkResult[name][key] != ""){
					...
					var tagName = chkResult[name]["obj"].tagName;

					// jquery select ui에 대응하기 위해서 추가 분기 처리 진행
					if(tagName == "SELECT"){
						$(chkResult[name]["obj"]).parent().find("span:first").focus();
					}else{
		...
		for(var name in chkResult){
			for(var key in chkResult[name]){
				if(chkResult[name][key] != ""){
					...
					nepa.alert("알림", chkResult[name][key], function(){ chkResult[name]["obj"].focus(); }, "/common/alert.do");
					return;
				}
			}
		}
	...
	getMsg : function(options, key, obj){
		...
		var title = typeof($(obj).attr("valMsg")) == "undefined" ? obj.name : $(obj).attr("valMsg");
